...
int DatReadCurrent(struct Datfp *fp,struct DatData *datdata) {
   ...
   do {
     if (ConvertReadShort(fp->datfp,&num_byte) !=0 || num_byte <= 0) {
       if (num_byte < 0){
           fprintf(stderr,"WARNING : DatReadCurrent : DatReadCurrent : num_byte < 0 in record header, potentially corrupted file.\n");
       }
       free(inbuf);
       return -1;
     }
   ...
   while ( num_byte > 0 ) {
      ...
      for(i = 0; i < datdata->PARMS.MPLGS ; ++i) {
         for(j=0 ; j < 2; ++j) {
	    if (xcf_data) datdata->xcfd[range][i][j] = dcmpr(inbuf_ptr);
	    else datdata->acfd[range][i][j] = dcmpr(inbuf_ptr);
            inbuf_ptr = inbuf_ptr + sizeof(int16);
            num_byte = num_byte - sizeof(int16);
         }
      }
