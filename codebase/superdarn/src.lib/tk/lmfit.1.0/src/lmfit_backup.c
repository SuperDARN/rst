...
double getguessex(struct RadarParm *prm,struct RawData *raw,
              struct FitData *fit, struct FitBlock *fblk, int print, int rang)
{
float minpwr  = 3.0;
  double skynoise = 0.;
  float sderr   = 3.0;
...
/* Generate models that will be used in the velocity determination */
  ...
  prm->noise.mean = skynoise;
    ...
    lag0pwr  = 10.0*log10((raw->acfd[0][rang*prm->mplgs] +
		...
		if(print)
			fprintf(stdout,"%d  %lf\n",rang,raw->acfd[0][rang*prm->mplgs]/sqrt(1.0*prm->nave));
    ...
    if((prm->cp == 3310 || prm->cp == 503 || prm->cp == -503) && prm->mplgs == 18)
    {
      for (L=0;L<prm->mplgs;L++)
      {
        ...
        if(print)
					fprintf(stdout,"%d  %lf  %lf  %d\n",lag,raw->acfd[0][rang*prm->mplgs+L],raw->acfd[1][rang*prm->mplgs+L],
																						(lagpwr[lag]>raw->acfd[0][rang*prm->mplgs]/sqrt(1.0*prm->nave)));
      }
    ...
    else
    {
      ...
      for (L=0;L<prm->mplgs;L++)
      {
        ...
        if(print)
					fprintf(stdout,"%d  %lf  %lf  %d\n",lag,raw->acfd[0][rang*prm->mplgs+L],raw->acfd[1][rang*prm->mplgs+L],
																						(availflg && lagpwr[lag]>raw->acfd[0][rang*prm->mplgs]/sqrt(1.0*prm->nave)));
      }
              ...
              struct FitData *fit, struct FitBlock *fblk, int print)
{
  ...
  for (R=0;R<prm->nrang;R++)
  {
    ...
    if((pwr_flg) && (availcnt>=minlag))
    {
      ...
      double model_guess = getguessex(prm,raw,fit,fblk,0,R);
      w_limit = model_guess;
