...
int phase_fitting(struct FitPrm *fitted_prms, LS_DATA* ls_data,
    ...
    status = do_phase_fit (ls_data->omega_loc, xflag, fitted_prms, acf, lag,BASE ,ls_data);
    ...
    if (!xflag && (status == 0)) {
        status = do_phase_fit (ls_data->omega_loc + ls_data->omega_err_loc,
                                xflag, fitted_prms, acf, lag, HIGH, ls_data);

        status = do_phase_fit (ls_data->omega_loc - ls_data->omega_err_loc, 
                                xflag, fitted_prms, acf, lag, LOW,ls_data);
