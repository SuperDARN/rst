...
*/
int phase_fit(double omega_guess, char xflag, struct FitPrm *fitted_prms,
...
void phase_fit_error(LS_DATA *ls_data, OMEGA location,
    ...
    wbar = wbar/nphi;
    dof_corrector=fitted_prms->mppul/fitted_prms->mplgs;
    if (xflag){
        if(nphi * dof_corrector >= 3){
            ls_data->phase_sdev = sqrt(e2/ls_data->sums->w * nphi/(nphi * dof_corrector - 2));
        }
        else{
            ls_data->phase_sdev = sqrt(e2/ls_data->sums->w * nphi);
        }
    }
    else{
        if(nphi * dof_corrector >= 2){
            ls_data->phase_sdev = sqrt(e2/ls_data->sums->w * nphi/(nphi * dof_corrector - 1));
        }
        else{
            ls_data->phase_sdev = sqrt(e2/ls_data->sums->w * nphi);
        }
...
int do_phase_fit (double omega_guess, char xflag, struct FitPrm *fitted_prms,
    ...
    phase_fit_error(ls_data, location, lag, xflag, fitted_prms);
