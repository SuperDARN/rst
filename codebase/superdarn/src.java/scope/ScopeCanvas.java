...
public class ScopeCanvas extends BaseCanvas {
  ...
  Color realcolor=Color.cyan;
  Color imagcolor=Color.red;
  ...
  public void plotData(ScopeData scopedata,PlotInfo plt) {
    ...
    for (y=plt.ymin;y<=plt.ymax;y+=plt.yminor) {
        ...
        bg.drawLine(ox,oy+ht-ay,ox+wd,oy+ht-ay);
    }
    ...
    for (y=plt.ymin;y<=plt.ymax;y+=plt.ymajor) {
        ay=ht*(y-plt.ymin)/(plt.ymax-plt.ymin);
        bg.drawLine(ox,oy+ht-ay,ox-2,oy+ht-ay);
	bg.drawLine(ox+wd,oy+ht-ay,ox+wd+2,oy+ht-ay);
	txt=Integer.toString(y);
	twdt=fm.stringWidth(txt);
        bg.drawString(txt,ox+wd+4,oy+ht-ay+5);
        bg.drawString(txt,ox-twdt-4,oy+ht-ay+5);
    }
    ...
    if (scopedata.buffer !=null) {  
	  ...
	  bx=wd*(x-plt.xmin)/(plt.xmax-plt.xmin);
          ay=ht*(scopedata.buffer[2*(x-1)]-plt.ymin)/(plt.ymax-plt.ymin);
          by=ht*(scopedata.buffer[2*x]-plt.ymin)/(plt.ymax-plt.ymin);
          if (ay<0) ay=0;
          if (by<0) by=0;
          if (ay>ht) ay=ht;
          if (by>ht) by=ht;

          bg.setColor(realcolor);
          bg.drawLine(ox+ax,oy+ht-ay,ox+bx,oy+ht-by);

          ay=ht*(scopedata.buffer[2*(x-1)+1]-plt.ymin)/(plt.ymax-plt.ymin);
          by=ht*(scopedata.buffer[2*x+1]-plt.ymin)/(plt.ymax-plt.ymin);
