...
public class PowerCanvas extends BaseCanvas {
  ...
  public void plotData(ScopeData scopedata,PlotInfo plt) {
    ...
    if (scopedata.buffer !=null) {  
	  ...
	  bx=wd*(x-plt.xmin)/(plt.xmax-plt.xmin);
          apwr=scopedata.buffer[2*(x-1)]*scopedata.buffer[2*(x-1)]+
	       scopedata.buffer[2*(x-1)+1]*scopedata.buffer[2*(x-1)+1];
	  bpwr=scopedata.buffer[2*x]*scopedata.buffer[2*x]+
	      scopedata.buffer[2*x+1]*scopedata.buffer[2*x+1];
